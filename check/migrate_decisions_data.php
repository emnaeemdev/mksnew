...
$decisionsSection = DocumentSection::where('slug', 'krarat')->first();
if (!$decisionsSection) {
    echo "❌ لم يتم العثور على قسم القرارات\n";
    exit(1);
}

echo "✅ تم العثور على قسم القرارات: {$decisionsSection->name} (ID: {$decisionsSection->id})\n";

// الحصول على الحقول المخصصة
$customFields = $decisionsSection->customFields->keyBy('name');
echo "✅ تم العثور على {$customFields->count()} حقل مخصص\n";

// ربط الحقول القديمة بالجديدة
$fieldMapping = [
    'decisions_number' => 'decisions_number',                    // رقم القرار
    'decisions_release_date' => 'decisions_release_date',        // تاريخ الإصدار
    'decisions_issuer_pub' => 'decisions_issuer_pub',            // جهة الإصدار
    'decisions_subject' => 'decisions_subject',                  // موضوع القرار
    'decisions_type' => 'decisions_type'                        // حالة القرار
];
...
try {
    ...
    foreach ($oldRecords as $index => $record) {
        try {
            ...
            $document->section_id = $decisionsSection->id;
            ...
            $document->views_count = $record['total_views'] ?? 0;
