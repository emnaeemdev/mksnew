...
@section('title', (app()->isLocale('ar') ? 'الكلمات المفتاحية للوثائق' : 'Document Keywords'))
...
<div class="container py-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('frontend.documents.index', app()->getLocale()) }}">{{ app()->isLocale('ar') ? 'الوثائق' : 'Documents' }}</a></li>
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
        <div>
            <h1 class="h4 mb-1 text-primary fw-bold">{{ app()->isLocale('ar') ? 'الكلمات المفتاحية للوثائق' : 'Document Keywords' }}</h1>
            <p class="text-muted mb-0">{{ app()->isLocale('ar') ? 'اختر كلمة لعرض الوثائق المرتبطة بها' : 'Choose a keyword to view related documents' }}</p>
        </div>
    ...
    @if($keywords->count())
        <div class="d-flex flex-wrap gap-2 mb-4">
            @foreach($keywords as $item)
                <a href="{{ route('frontend.documents.keywords.show', [app()->getLocale(), $item->slug]) }}" class="btn btn-outline-primary rounded-pill">
                    {{ $item->name }}
