...
$destoryaSection = DocumentSection::where('slug', 'judgments-of-the-supreme-constitutional-court')->first();
if (!$destoryaSection) {
    echo "❌ لم يتم العثور على قسم القرارات\n";
    exit(1);
}

echo "✅ تم العثور على قسم القرارات: {$destoryaSection->name} (ID: {$destoryaSection->id})\n";

// الحصول على الحقول المخصصة
$customFields = $destoryaSection->customFields->keyBy('name');
echo "✅ تم العثور على {$customFields->count()} حقل مخصص\n";

// ربط الحقول القديمة بالجديدة
$fieldMapping = [
    'destorya_number' => 'destorya_number',                    // رقم القرار
    'destorya_date' => 'destorya_date',        // تاريخ الإصدار
    'destorya_type' => 'destorya_type',            // جهة الإصدار
    'destorya_year' => 'destorya_year',                  // موضوع القرار
...
try {
    ...
    foreach ($oldRecords as $index => $record) {
        try {
            ...
            $document->section_id = $destoryaSection->id;
            $document->user_id = $defaultUser->id;
            $document->title = $record['title_ar'];
            $document->content = $record['destorya_dec'] ?? '';
