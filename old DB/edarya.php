...
$edaryaSection = DocumentSection::where('slug', 'judgments-of-the-supreme-administrative-court')->first();
if (!$edaryaSection) {
    echo "❌ لم يتم العثور على قسم القرارات\n";
    exit(1);
}

echo "✅ تم العثور على قسم القرارات: {$edaryaSection->name} (ID: {$edaryaSection->id})\n";

// الحصول على الحقول المخصصة
$customFields = $edaryaSection->customFields->keyBy('name');
echo "✅ تم العثور على {$customFields->count()} حقل مخصص\n";

// ربط الحقول القديمة بالجديدة
$fieldMapping = [
    'edarya_number' => 'edarya_number',                    // رقم القرار
    'edarya_date' => 'edarya_date',        // تاريخ الإصدار
    'edarya_type' => 'edarya_type',            // جهة الإصدار
    'edarya_year' => 'edarya_year',                  // موضوع القرار
...
try {
    ...
    foreach ($oldRecords as $index => $record) {
        try {
            ...
            $document->section_id = $edaryaSection->id;
            $document->user_id = $defaultUser->id;
            $document->title = $record['title_ar'];
            $document->content = $record['edarya_dec'] ?? '';
